...
<script>
    $('.btn-edit').click(function() {
        ...
        $.each(data.translations, function(index, category) {
            $('#form-edit #input-name-' + category.locale).val(category.name ?? '');
        });
...
<section class="section">
    <div class="section-header">
        ...
        <div class="section-header-button">
            <button class="btn btn-primary" data-toggle="modal" data-target="#modal-add">Add Categories</button>
        </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="float-right">
                            <form action="{{ route('admin-blog.category.index') }}">
                                    ...
                                    <tr>
                                        <td>
                                            <span class="d-inline-block py-2" style="max-width: 150px;">
                                                <img src="{{ $category->image ? asset('upload/images/blog-category/' . $category->image) : 'https://via.placeholder.com/700x350' }}" alt="Categories Image" class="img-fluid w-100">
                                            </span>
                                        </td>
                                        <td>{{ $category->name }}</td>
                                        <td>
                                            <button class="btn btn-primary btn-edit" data-params="{{ $category }}" data-url="{{ route('admin-blog.category.update', $category->id) }}"><i class="far fa-edit"></i></button>
                                            <button class="btn btn-danger btn-delete" type="button" data-url="{{ route('admin-blog.category.delete', $category->id) }}"><i class="fa fa-trash"></i></button>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
            ...
            <form action="{{ route('admin-blog.category.store') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <ul class="nav nav-tabs">
                        ...
                        <li class="nav-item">
                            <a class="nav-link {{ $loop->iteration == 1 ? 'active' : '' }}" data-toggle="tab" href="#category-{{ $locale }}">{{ $property['name'] }}</a>
                        </li>
                        @endforeach
                    </ul>
                    <div class="tab-content">
                        @foreach(LaravelLocalization::getSupportedLocales() as $locale => $property)
                        <div id="category-{{ $locale }}" class="tab-pane fade {{ $loop->iteration == 1 ? 'show active' : '' }}">
                            <div class="form-group">
                                <label>Name</label>
            ...
            <form action="{{ old('url') }}" method="post" enctype="multipart/form-data" id="form-edit">
                ...
                <div class="modal-body">
                    ...
                    <ul class="nav nav-tabs">
                        @foreach(LaravelLocalization::getSupportedLocales() as $locale => $property)
                        <li class="nav-item">
                            <a class="nav-link {{ $loop->iteration == 1 ? 'active' : '' }}" data-toggle="tab" href="#category-edit-{{ $locale }}">{{ $property['name'] }}</a>
                        </li>
                        @endforeach
                    </ul>
                    <div class="tab-content">
                        @foreach(LaravelLocalization::getSupportedLocales() as $locale => $property)
                        <div id="category-edit-{{ $locale }}" class="tab-pane fade {{ $loop->iteration == 1 ? 'show active' : '' }}">
                            <div class="form-group">
                                <label>Name</label>
